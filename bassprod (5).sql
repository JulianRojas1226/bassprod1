...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 17-09-2024 a las 23:32:05
...
-- Procedimientos
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `act_cant` (IN `p_id_producto` INT, IN `p_cantidad` INT)   BEGIN
    
    INSERT INTO producto (id_producto, cantidad)
...
--
DELIMITER $$
CREATE TRIGGER `d_cant` AFTER INSERT ON `detalles_pedido` FOR EACH ROW BEGIN
    DECLARE cant_p INT;
    -- Obtener la cantidad actual del producto relacionado
...
--
-- Estructura Stand-in para la vista `mesa_reservada`
-- (Véase abajo para la vista actual)
--
...
(2, '2024-09-17 04:48:24', 2, 10, NULL);
...
--
-- Estructura para la vista `mesa_reservada`
--
DROP TABLE IF EXISTS `mesa_reservada`;
